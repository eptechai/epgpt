...
def delete_index(channel, method: Basic.Deliver, properties: BasicProperties, body):
    ...
    try:
        ...
        storage.delete_file(remote_path=f"{conversation_id}/{filename}")

        for file_type in ["faiss", "pkl"]:
            storage.delete_file(remote_path=f"{conversation_id}/{filename}.{file_type}")
        status = "DELETED"
        _logger.info(f"Successfully deleted file: {filename} ({attachment_id}) | {conversation_id}")
    except Exception:
        status = "ERRORED"
        _logger.exception(f"Unable to delete file: {filename} ({attachment_id}) | {conversation_id}")
    finally:
